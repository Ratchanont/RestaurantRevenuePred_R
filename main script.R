...
set.seed(101)
sample = sample.split(train_final, SplitRatio = 0.7)
head(sample)

tr = subset(train_final, sample == TRUE)
te = subset(train_final, sample == FALSE)

mod <- lm(revenue ~ age,data = tr)
summary(mod)

mod1 <- lm(revenue ~ age+typeLabel,data = tr)
summary(mod1)

mod_final <- lm(revenue ~ age,data = tr)
summary(mod_final)
...
mod_final2 <- lm(revenue ~ age,data = train_final)
